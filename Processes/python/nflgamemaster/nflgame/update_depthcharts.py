...
urls = {
    'depthchart': 'http://www.nfl.com/teams/depthchart?team=%s',
}
...
def depthchart_soup(team):
    resp, content = new_http().request(urls['depthchart'] % team, 'GET')
    if resp['status'] != '200':
        return None
...
def run():
    ...
    def fetch(team):
        return team, depthchart_soup(team)
    for i, (team, soup) in enumerate(pool.imap(fetch, teams), 1):
        progress(i, len(teams))

        if soup is None:
            errors.append('Could not get depthchart for team %s' % team)
            continue
