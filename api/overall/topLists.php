...
// crime
$result2 = $db->query('SELECT Category, COUNT(Category) AS arrest_count FROM `arrest_stats` '. $date_range .'GROUP BY Category ORDER BY arrest_count DESC' . $limit);

$megaresult[] = gather_results($result2);

// players
$result = $db->query('SELECT Name, Position, COUNT(Name) AS arrest_count FROM `arrest_stats` '. $date_range .'GROUP BY Name HAVING arrest_count > 1 ORDER BY arrest_count DESC' . $limit);

$megaresult[] = gather_results($result);

// position
$result3 = $db->query('SELECT Position, COUNT(Position) AS arrest_count FROM `arrest_stats` '. $date_range .'GROUP BY Position ORDER BY arrest_count DESC' . $limit);
