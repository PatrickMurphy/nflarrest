...
$result = $db->query('SELECT MONTH(Date) AS Month, YEAR(Date) AS Year, COUNT(Team) AS arrest_count FROM `arrest_stats` WHERE Team = \''. $id .'\''. $date_range .' GROUP BY YEAR(Date), MONTH(Date) ORDER BY Date ASC' . $limit);
