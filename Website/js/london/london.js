var drng;
var mainChartReturned = false;
var last_start_pos = 0,
		listsReturnCount = 0,
		listsReturned = false;
var sunToggle = false, rainToggle = false;
...
$( window ).load(function() {
	dateLondonRangeController.init(function(newDateRange){
		drng = newDateRange;
		setupChart();
		$('#toggleRain').click(function(){
			console.log(rainToggle);
...
function getOverallChartData(callback){
	console.log('creating chart. Sun: '+sunToggle + ', Rain: '+rainToggle);
	var xFields = ['x'];
	var highsCol = ['High'];
	var lowsCol = ['Low'];
	var frost = ['Days of Frost'];
	var rain = ['Rainfall'];
	var sun = ['Hours of Sun'];
	...
	var thisRawData = rawData.slice(0).splice(date2key(drng.getStart()), (date2key(drng.getEnd())-date2key(drng.getStart())));
	for(var key in thisRawData){
		var element = thisRawData[key];
		...
		sun.push(element.sun/17);
		rain.push(element.rain/10);
	}
	var tempCols = [xFields, highsCol, lowsCol,frost,resp];
	if(sunToggle === true){
			tempCols.push(sun);
		}
	console.log(rainToggle);
	...
	var returnData = {'columns':tempCols,'groups':groups};
