...
$query = 'SELECT Season as season, sum(CASE WHEN ArrestSeasonState = \'InSeason\' THEN 1 ELSE 0 END) as InSeason,sum(CASE WHEN ArrestSeasonState = \'OffSeason\' THEN 1 ELSE 0 END) as OffSeason FROM `ArrestsCacheTable` '.$date_range.' GROUP BY Season'.$limit;
